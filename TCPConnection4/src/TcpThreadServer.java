...
public class TcpThreadServer {
  ...
  private BufferedReader FilegetReader(Socket socket) throws IOException {
    // 获得输入流缓冲区的地址
  ...
  class Handler implements Runnable {
    ...
    public void run() {
      ...
      try {
        ...
        pw.println("注册成功！您的昵称是：" + hostName + "\n");
        pw.println("请您选择聊天状态：G群聊/P私信\n");
          ...
          //选择群聊
            ...
            while ((msg = br.readLine()) != null) {
              if (!"E".equals(msg) && users.size() != 1) {
                ...
                sendToMembers("已退出聊天室！！", localName, socket);
                pw.println("您已退出群组聊天室！");
                break;
              }
            }

          } else if ("D".equalsIgnoreCase(msg.trim())) {
            pw.println("请您选择要传输的文件");
          } else {
            pw.println("请选择聊天状态！");}
        }
      } catch (IOException e) {
